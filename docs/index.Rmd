...
- The required readings can be downloaded from: https://github.com/alburezg/EDSD_2023_kinship_demography/tree/main/readings [DO WE HAVE ANY CHANGES?]
...
https://population.un.org/wpp/Download/Standard/CSV/}
    1 - Fertility: 1950-2100, single age;
    2 - Life Tables: 1940 - 2021 for males and females

In the docs folder of this project, create a folder called unwppdata and save it to store the 3 files that we have downloaded.
...
##### 6.1 Download data in 2015 from the country of interest and build the 'Keyfitz' kinship diagram for a Focal girl aged 5 yo in a time-invariant population. Discuss the result with what we saw for Sweden and Brazil.
...
# Run kinship models
swe_2015 <- kin(p = swe_surv_2015, f = swe_asfr_2015, output_kin = output_kin, time_invariant = TRUE)
```
...
##### 2.3. Age distribution of living kin
...
Optional: Do the same exercises using the country of your choice by downloading the WPP data.
...
In today's session we will see how to application for the one-sex; time-variant model and two-sex;time-variant model.
...
##### 3.1. Expected kin for your country of choice

###### Exercise 1

Using the `data` for your own country, reshape the fertility and survival data in the same way in which we reshaped the fertility data above. Plot the evolution of mortality over time and age, or create a table that summaries that. You can use the plot of $q_x$ values over age for all periods that we did for Sweden as inspiration (using different colours for each period).
...
Run a time-variant kinship model for your country. Create only results for two years: 1950 and 2015 (i.e. apply a period perspective). Focusing on a Focal aged 50, compare both years in terms of (a) total living and and (b) mean age. **Answer:** Which type of kin shows the largest differences across those two years?

```{r, eval= F, include=F}
# Write your code here
```

### 4.Age-classified two-sex kinship models and do some exercises.
...
##### 4.2 Time-invariant two-sex kinship models
...
##### 4.3 Time-variant two-sex kinship models
...
**E1** Download data for your country from United Nations-Population Division. But this time also use the parameter `sex = "Male"` because you will need male-specific survival patterns by age and time. Don´t forget to reshape data to matrix format as we did yesterday. Assume for this exercise the same fertility pattern for males than females and answer:
...
**E12** What about countries we don't have information on male fertlity?
